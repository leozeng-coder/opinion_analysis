...
class BilibiliCrawler(AbstractCrawler):
    ...
    async def search_by_keywords(self):
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while (page - start_page + 1) * bili_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                ...
                utils.logger.info(f"[BilibiliCrawler.search_by_keywords] Sleeping for {sleep_time:.2f} seconds after page {page-1}")
            ...
            for day in pd.date_range(start=config.START_DAY, end=config.END_DAY, freq="D"):
                ...
                while True:
                    ...
                    try:
                        ...
                        sleep_time = utils.get_random_sleep_time(config.CRAWLER_MAX_SLEEP_SEC)
                        await asyncio.sleep(sleep_time)
                        utils.logger.info(f"[BilibiliCrawler.search_by_keywords_in_time_range] Sleeping for {sleep_time:.2f} seconds after page {page-1}")
        ...
        async with semaphore:
            try:
                ...
                await self.bili_client.get_video_all_comments(
                    video_id=video_id,
                    crawl_interval=config.CRAWLER_MAX_SLEEP_SEC,  # 传递配置，让客户端内部每次生成随机值
    ...
    async def get_creator_videos(self, creator_id: int):
        ...
        while True:
            ...
            sleep_time = utils.get_random_sleep_time(config.CRAWLER_MAX_SLEEP_SEC)
            await asyncio.sleep(sleep_time)
            utils.logger.info(f"[BilibiliCrawler.get_creator_videos] Sleeping for {sleep_time:.2f} seconds after page {pn}")
            pn += 1
    ...
    async def get_bilibili_video(self, video_item: Dict, semaphore: asyncio.Semaphore):
        ...
        sleep_time = utils.get_random_sleep_time(config.CRAWLER_MAX_SLEEP_SEC)
        await asyncio.sleep(sleep_time)
        utils.logger.info(f"[BilibiliCrawler.get_bilibili_video] Sleeping for {sleep_time:.2f} seconds after fetching video {aid}")
        if content is None:
            return
