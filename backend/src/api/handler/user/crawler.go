...
func (h *CrawlerHandler) RunNow(c *gin.Context) {
	...
	if err := h.crawler.CreateRunLog(&logRow); err != nil {
...
func (h *CrawlerHandler) runSubprocess(logID uint, root, py, spiders, filterJSON string) {
	...
	rows, errFinish := h.crawler.FinishRunLog(logID, finish)
	if errFinish != nil {
		log.Printf("[crawler task] subprocess_finish_db_error run_id=%d status=%s elapsed=%v err=%v", logID, status, elapsed, errFinish)
	} else if rows == 0 {
